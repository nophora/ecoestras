...
export default function Product() {
  ...
  useEffect(() => {
    ...
    const dataStr = localStorage.getItem('test_product');
    if (dataStr) {
      try {
        const data = JSON.parse(dataStr);
        setProduct(data);

        // Dynamically initialize selectedVariant
        const initialVariants = data.variants?.map((v: any) => {
          const key = Object.keys(v)[0];
          return { [key]: v[key][0] };
        }) || [];
        setSelectedVariant(initialVariants);
      } catch (e) {
        console.error('Error loading test product:', e);
      }
    }
  ...
  const buyButton = async () => {
    if (product) {
      console.log(`Test Product : buying the product without adding it to the cart`)
    }
  };
  ...
  const AddToCartButton = async () => {
    if (product) {
      console.log(`Test Product : adding the product to the cart`)
  ...
  const subtotal = cartItems.reduce((acc, item) => acc + (item.pricing?.customer_totalprice || 0), 0);

  const carterToCheckoutButton = async () => {
    console.log(`Test Product : moving the product to the checkout`)
  };
  ...
  return (
    <main className="min-h-screen bg-white font-sans selection:bg-black selection:text-white uppercase tracking-tight overflow-x-hidden">
      ...
      {/* Floating Action Bar */}
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[2000] w-full max-w-md px-4">
        <div className="bg-white/80 backdrop-blur-xl border border-gray-100 p-6 rounded-[2.5rem] shadow-2xl flex gap-4">
          <button
            onClick={() => router.push('/dashboard')}
            className="flex-1 py-4 rounded-full border-2 border-black bg-white text-black font-black uppercase tracking-widest text-[10px] hover:bg-gray-50 transition-all flex items-center justify-center gap-2"
          >
            <Trash2 size={14} />
            Edit Product
          </button>
          <button
            onClick={async () => {
              try {
                ...
                if (res) {
                  localStorage.removeItem('test_product');
            ...
            className="flex-1 py-4 rounded-full bg-black text-white font-black uppercase tracking-widest text-[10px] shadow-xl hover:bg-gray-900 transition-all flex items-center justify-center gap-2"
          >
